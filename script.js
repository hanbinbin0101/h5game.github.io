document.addEventListener('DOMContentLoaded', function() {
    ...
    if (mobileMenuButton && mobileMenu) {
        console.log('找到移动菜单按钮和菜单');
        
        mobileMenuButton.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            console.log('点击了移动菜单按钮');
    ...
    const translations = {
        'en': {
            ...
            'Sports': 'Sports',
            'Arcade': 'Arcade',
            'Immerse Yourself in Ultimate Gaming Experience': 'Immerse Yourself in Ultimate Gaming Experience',
            'Discover a world of endless possibilities with our curated collection of top-rated games designed for every type of player.': 'Discover a world of endless possibilities with our curated collection of top-rated games designed for every type of player.',
            '8 Ball Billiards Classic': '8 Ball Billiards Classic',
            'Stickman Destruction Heroes': 'Stickman Destruction Heroes',
            'Count Masters Stickman': 'Count Masters Stickman',
            'Enjoy the classic billiards game experience, challenge your opponents, and hone your aiming skills and strategies.': 'Enjoy the classic billiards game experience, challenge your opponents, and hone your aiming skills and strategies.',
            'Control stickman heroes to perform various crazy actions and destructive challenges, and enjoy the fun of physics simulation.': 'Control stickman heroes to perform various crazy actions and destructive challenges, and enjoy the fun of physics simulation.',
            'Collect and increase your stickman army, break through obstacles, and defeat opponents in this exciting multiplayer crowd control game.': 'Collect and increase your stickman army, break through obstacles, and defeat opponents in this exciting multiplayer crowd control game.',
            'Hover to load game': 'Hover to load game',
            'Fullscreen mode': 'Fullscreen mode',
            'Select a game to start': 'Select a game to start',
        ...
        'zh': {
            ...
            'Sports': '体育',
            'Arcade': '街机',
            'Immerse Yourself in Ultimate Gaming Experience': '沉浸在终极游戏体验中',
            'Discover a world of endless possibilities with our curated collection of top-rated games designed for every type of player.': '探索由我们精心挑选的适合各类玩家的顶级游戏所带来的无限可能。',
            '8 Ball Billiards Classic': '经典8球台球',
            'Stickman Destruction Heroes': '火柴人毁灭英雄',
            'Count Masters Stickman': '火柴人计数大师',
            'Enjoy the classic billiards game experience, challenge your opponents, and hone your aiming skills and strategies.': '享受经典的台球游戏体验，挑战对手，磨练你的瞄准技巧和策略。',
            'Control stickman heroes to perform various crazy actions and destructive challenges, and enjoy the fun of physics simulation.': '控制火柴人英雄执行各种疯狂的动作和破坏性挑战，享受物理模拟的乐趣。',
            'Collect and increase your stickman army, break through obstacles, and defeat opponents in this exciting multiplayer crowd control game.': '收集和增加你的火柴人大军，冲破障碍，在这款刺激的多人群体控制游戏中击败对手。',
            'Hover to load game': '悬停加载游戏',
            'Fullscreen mode': '全屏模式',
            'Select a game to start': '选择游戏开始',
    ...
    // 当前语言
    let currentLang = localStorage.getItem('gameverse-language') || 'en';
    
    // 初始化时立即应用语言设置
    function initLanguage() {
        if (!currentLanguage || !mobileCurrentLanguage) {
            console.error('语言切换按钮未找到，正在重试...');
        ...
        mobileCurrentLanguage.textContent = currentLang.toUpperCase();
    ...
    // 桌面版语言切换按钮点击事件
    if (languageToggle) {
        console.log('已找到桌面版语言切换按钮');
        languageToggle.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            console.log('点击了桌面版语言切换按钮');
            toggleLanguage();
        });
    } else {
        console.warn('未找到桌面版语言切换按钮');
        // 尝试在DOM加载后延迟查找并添加事件
        setTimeout(function() {
            const delayedToggle = document.getElementById('language-toggle');
            if (delayedToggle) {
                console.log('延迟找到桌面版语言切换按钮');
                delayedToggle.addEventListener('click', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    console.log('点击了桌面版语言切换按钮');
                    toggleLanguage();
    ...
    if (mobileLanguageToggle) {
        console.log('已找到移动版语言切换按钮');
        mobileLanguageToggle.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            console.log('点击了移动版语言切换按钮');
            toggleLanguage();
        });
    } else {
        console.warn('未找到移动版语言切换按钮');
        // 尝试在DOM加载后延迟查找并添加事件
        setTimeout(function() {
            const delayedMobileToggle = document.getElementById('mobile-language-toggle');
            if (delayedMobileToggle) {
                console.log('延迟找到移动版语言切换按钮');
                delayedMobileToggle.addEventListener('click', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    console.log('点击了移动版语言切换按钮');
                    toggleLanguage();
    ...
    function toggleLanguage() {
        console.log('正在切换语言，当前语言:', currentLang);
        currentLang = currentLang === 'en' ? 'zh' : 'en';
        localStorage.setItem('gameverse-language', currentLang);
        
        // 更新语言显示
        if (currentLanguage) currentLanguage.textContent = currentLang.toUpperCase();
        if (mobileCurrentLanguage) mobileCurrentLanguage.textContent = currentLang.toUpperCase();
        
        // 应用新语言
        applyLanguage(currentLang);
        
        console.log('语言已切换至:', currentLang);
    }
    ...
    function applyLanguage(lang) {
        ...
        try {
            ...
            document.querySelectorAll('button').forEach(button => {
                // 跳过语言切换按钮
                if (button.id === 'language-toggle' || button.id === 'mobile-language-toggle') {
                    return;
                }
                ...
                // 如果找到文本并有对应翻译，则更新
                if (buttonText && translations[lang][buttonText]) {
                    for (let node of button.childNodes) {
                        if (node.nodeType === 3) { // 文本节点
                            const text = node.textContent.trim();
                            if (text) {
                                node.textContent = node.textContent.replace(text, translations[lang][buttonText]);
                                break;
                            }
                        }
                    }
            ...
            document.querySelectorAll('.category-tab').forEach(tab => {
                ...
                if (translations[lang][text]) {
                    tab.textContent = translations[lang][text];
                }
            });
            
            // 翻译游戏卡片描述
            document.querySelectorAll('.text-apple-gray-700').forEach(desc => {
                const text = desc.textContent.trim();
                if (translations[lang][text]) {
                    desc.textContent = translations[lang][text];
                }
            });
